/*
 * Sample program of R^2 and color library for both BGI and XBGI < attract2.c >
 *
...
void bifurcate(void)
{
  ...
  for (b=0; b<BTIME; b++) {
    ...
    iterate();
    if (MAXCOLOR>1)
      putpixels_(itime, p, b%(MAXCOLOR-1)+1); /* 論理座標への描画 */
    else
      putpixels_(itime, p, MAXCOLOR); /* 論理座標への描画 */
  }
}
