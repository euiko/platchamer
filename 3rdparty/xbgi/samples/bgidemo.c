...
int main()
{
  ...
  PutPixelDemo();
  PutImageDemo();
  FloodFillDemo(); /***** it's buggy on PseudoColor, TrueColor in X *****/
  Bar3DDemo();
  BarDemo();
  RandomBars();
  SectorDemo();
...
void TextDemo(void)
{
  ...
  for( font=0 ; font<NFONTS ; ++font ){ /* For each of the four fonts   */
...
/*                                                                      */
/*      SECTORDEMO: Display a random pattern of sectors on the screen */
/*      until the user says enough.                                     */
/*                                                                      */

void SectorDemo(void)
{
  int xradius, yradius;                 /* Maximum radius allowed       */
  int sangle, eangle;                   /* Random end angle of Sector   */
  int c;
  ...
  while( !kbhit() ){                    /* Repeat until a key is hit    */
    setcolor( c=random( MaxColors - 1 ) + 1 );/* Randomly select a color  */
    setfillstyle( SOLID_FILL, c );
    sangle = random( 358 ) + 1;         /* Select an start angle        */
    eangle = random( 358 ) + 1;         /* Select an end angle          */
    xradius = random(MaxX/10);          /* Determine the maximum xradius */
    yradius = random(MaxY/10);          /* Determine the maximum yradius */

    sector( random(MaxX), random(MaxY), sangle, eangle,
	   xradius, yradius );
  }                                     /* End of WHILE not KBHIT       */
...
/*                                                                      */
/*      FLOODFILLDEMO: Display lines and fill using floodfill commands. */
/*                                                                      */
void FloodFillDemo(void)
{
  int i, x, y, c; 
  ...
  while ( !kbhit() ) {
    x = random(MaxX/10);
    y = random(MaxY/10);
    c = random(MaxColors-1);
    setfillstyle(SOLID_FILL, c+1);
    floodfill(10*x+1,10*y+1, WHITE);
  }
  Pause();                              /* Wait for user's response     */
}
...
void PaletteDemo(void)
{
  ...
  for( j=0 ; j<10 ; ++j ){              /* For 10 rows of boxes         */
    for( i=0 ; i<15 ; ++i ){            /* For 15 columns of boxes      */
      ...
      if ( MaxColors > 2 ) /***** Modified for MONO *****/
...
void PolyDemo(void)
{
  ...
  while( !kbhit() ){                    /* Repeat until a key is hit    */
    ...
    drawpoly( MaxPts, (int far *)poly );    /* Draw the actual polygon      */
  }                                     /* End of WHILE not KBHIT       */
