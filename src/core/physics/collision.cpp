...
#include "shapes/circle_collider.hpp"
#include "shapes/polygon_collider.hpp"
#include "../../types/vect2.hpp"

CollisionCallback Dispatch[ColliderShape::colliderCount][ColliderShape::colliderCount] =
{
  {
...
float FindAxisLeastPenetration( uint32_t *faceIndex, PolygonCollider *A, PolygonCollider *B )
{
  ...
  for(uint32_t i = 0; i < A->m_vertexCount; ++i)
  {
    ...
    Vect2 s = B->GetSupport( -n );

    // Retrieve vertex on face from A, transform into
    // B's model space
    Vect2 v = A->m_vertices[i];
...
void FindIncidentFace( Vect2 *v, PolygonCollider *RefPoly, PolygonCollider *IncPoly, uint32_t referenceIndex )
{
  Vect2 referenceNormal = RefPoly->m_normals[referenceIndex];
...
{
  PolygonCollider *A = reinterpret_cast<PolygonCollider *>(a->shape);
  PolygonCollider *B = reinterpret_cast<PolygonCollider *>(b->shape);
  m->contact_count = 0;
  ...
  Vect2 v1 = RefPoly->m_vertices[referenceIndex];
  referenceIndex = referenceIndex + 1 == RefPoly->m_vertexCount ? 0 : referenceIndex + 1;
  Vect2 v2 = RefPoly->m_vertices[referenceIndex];
